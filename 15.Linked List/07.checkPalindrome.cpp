...
class LinkedList {
    ...
    public:
        ...
        Node* find_middle_node() {
            ...
            count = (count & 1) ? (count+1) >> 1 : count >> 1;
            temp = head;
            while (--count) {
                temp = temp->next;
            }
        ...
        void reverse_right_half_from_node(Node* current) {
            Node* previous = NULL;
            while (current) {
        ...
        void rearrange_LL() {
            Node *previous=NULL, *current=tail;
            while (current) {
                Node* next = current->next;
                current->next = previous;
                previous = current;
        ...
        bool is_it_palindrome() {
            if (length <= 1) return true;
            // find middle node
            Node* middle = find_middle_node();
            // reverse the right side half
            reverse_right_half_from_node(middle->next);
            // check for palindrome
            bool ans = check_palindrome();
            // rearrange the LL
            rearrange_LL();
            return ans;
        };
...
int main() {
    ...
    cout << "Is it a palindrome? => " << LL->is_it_palindrome() << endl;
    LL->printLL();
