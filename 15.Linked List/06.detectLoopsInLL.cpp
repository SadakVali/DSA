...
class LinkedList {
    ...
    public:
        ...
        void create_a_loop_at_node_p(int position) {
            Node* temp = head;
            while (--position) temp = temp->next;
            tail->next = temp;
        }
        Node* does_loop_exist() {
        ...
        Node* loop_starting_node() {
            Node* fast = does_loop_exist();
            Node* slow = head;
            if (!fast) return NULL;
            while (fast != slow) {
                fast = fast->next;
        ...
        void remove_loop() {
            // tail->next = NULL; // OR 
            Node* temp = loop_starting_node();
            Node* iter_node = temp;
            while (iter_node->next != temp) iter_node = iter_node->next;
            iter_node->next = NULL;
        }
    private:
...
int main() {
    ...
    LL->create_a_loop_at_node_p(3);
    if (LL->does_loop_exist()) cout << "Loop Exists..." << endl;
    else cout << "No Loop..." << endl;
    cout << "Loop Starting Node data : " << LL->loop_starting_node()->data << endl;
    LL->remove_loop();
    LL->printLL();
